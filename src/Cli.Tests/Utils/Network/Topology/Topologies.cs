...
/// Returns network topologies that can be used with AgentTestHarness.CreateFromTopologyAsync().
/// </summary>
internal static class Topologies {
  /// <summary>
  /// Two agents, each on different subnets, coordinator on its own subnet.
  /// The coordinator reaches both agents directly, while the agent subnets remain isolated.
  /// Tests basic distributed scanning with coordinator-local participation.
  /// </summary>
  public static NetworkTopologyBuilder TwoAgentsDisjointSubnetsWithCoordinator() {
    return new NetworkTopologyBuilder()
      .AddSubnet(
        "coordinator-subnet",
        "192.168.0.0/24",
        [( "192.168.0.100", "Coordinator device" )],
        out var coordinatorSubnet
      )
      .AddSubnet(
        "agent1-subnet",
        "192.168.10.0/24",
      ...
      .AddAgent( "agent_a", agent1Subnet )
      .AddAgent( "agent_b", agent2Subnet )
      .WithCoordinatorSubnet( coordinatorSubnet );
  }

  /// <summary>
  /// Two agents, each on different subnets, coordinator has no local interfaces.
  /// Tests agent-only scanning without coordinator-local interfaces.
  /// </summary>
  public static NetworkTopology TwoAgentsDisjointSubnetsNoCoordinatorSubnet() {
    return new NetworkTopologyBuilder()
      .AddSubnet(
        "agent1-subnet",
        "192.168.10.0/24",
  ...
  /// <summary>
  /// Two agents on the SAME subnet with coordinator on different subnet.
  /// Tests coordinator assignment when multiple agents report an overlapping network.
  /// </summary>
  public static NetworkTopology TwoAgentsOverlappingSubnet() {
    return new NetworkTopologyBuilder()
      ...
      .AddAgent( "agent_a", sharedSubnet )
      .AddAgent( "agent_b", sharedSubnet )
      .WithCoordinatorSubnet( coordinatorSubnet )
      .Build();
  }
  ...
  public static NetworkTopology SingleAgentEmptyResults() {
    return new NetworkTopologyBuilder()
      ...
      .AddAgent( "agent_a", emptySubnet )
      .Build();
  }

  /// <summary>
  /// Coordinator + one agent on different subnets.
  /// Tests mixed coordinator-local and remote scanning.
  /// </summary>
  public static NetworkTopology MixedCoordinatorAndAgents() {
    return new NetworkTopologyBuilder()
      .AddSubnet(
        "coordinator-subnet",
        "192.168.0.0/24",
      ...
      .AddAgent( "agent_a", agentSubnet )
      .WithCoordinatorSubnet( coordinatorSubnet )
      .WithFirewall( firewall => {
        firewall.DefaultPolicy = FirewallAction.Deny;
        firewall.Allow(
          FirewallTarget.Subnet( "coordinator-subnet" ),
          FirewallTarget.Subnet( "coordinator-subnet" )
        );
        firewall.Allow( FirewallTarget.Subnet( "agent-subnet" ), FirewallTarget.Subnet( "agent-subnet" ) );
      } )
      .Build();
  }
  ...
  public static NetworkTopology GuestNetworkIsolation() {
    return new NetworkTopologyBuilder()
      ...
      .AddAgent( "agent_guest", guestSubnet )
      .WithFirewall( fw => {
  ...
  public static NetworkTopology ManagementNetworkWithCidr() {
    return new NetworkTopologyBuilder()
      ...
      .AddAgent( "agent_mgmt", mgmtSubnet )
      .AddAgent( "agent_prod1", prod1Subnet )
      .AddAgent( "agent_prod2", prod2Subnet )
      .WithFirewall( fw => {
