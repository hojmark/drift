...
public class PingNetworkScanner2( IPingTool pingTool ) {
  ...
  public async Task<ScanResult> ScanAsync(
    ...
    foreach ( var cidr in cidrs ) {
      await PingScanAsync( pingReplies, cidr, logger, maxPingsPerSecond, onProgress, cancellationToken, pingProgress,
        onProgressNew );
    }
    ...
    onProgressNew.Invoke( progressBuilder.Build( "Reading ARP cache" ) );*/
  ...
  private async Task PingScanAsync( ConcurrentBag<(string Ip, bool Success, string? Hostname)> results,
    ...
    logger.LogDebug( "Starting ping scan for CIDR block {Cidr} ({AddressCount} addresses)", cidr, total );
    /*progressBuilder.AddStep( NetworkScanStep.PingScanning, new NetworkScanData { Subnets = null } );
    onProgressNew.Invoke(
      progressBuilder.Build( "Starting ping scan for CIDR block {Cidr} ({AddressCount} addresses)" )
    );*/
    ...
    var pingTasks = ipRange.Select( async ip => {
      ...
      try {
        ...
        string? hostname = "";
        if ( success ) {
          logger.LogDebug( "Ping to {Ip} succeeded", ip );
          /*progressBuilder.UpdateStep( NetworkScanStep.PingScanning, 40, new NetworkScanData { Subnets = null } );
          onProgressNew.Invoke(
            progressBuilder.Build( $"Ping to {ip} succeeded" )
          );*/
